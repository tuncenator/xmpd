...
| `bin/xmpctl` | CLI client. Subcommand dispatch via top-level `if`/`elif` blocks; Unix socket IPC; `format_track_fzf` for ANSI rendering; `colorize` helpers. | `cmd_history_json(args)` handles `--mode`, `--since SPEC`, `--limit`, `--format`. Client-side SPEC translation (`30d`/`7d`/`1h`/`all` to ISO). `format_played_at(iso_str)` renders ISO as `May-12 19:39`. `format_duration_seconds(seconds)` renders as `m:ss`. Dispatch in `main()`. Help text includes `history-json`. New `history-backfill` subcommand also planned here (Phase 6). |
