...
>
> Last updated by: Checkpoint 6 -- Phase 8 integration testing with 4 surgical fixes (2026-05-13)
...
| `tests/test_history_syncer.py` | HistorySyncer tests: 14 tests across precheck, wire format, single-flight, failure paths, nudge, SSH config bypass. | Uses `history_store_temp` + `mock_ssh_bidir` fixtures from conftest.py. Phase 8 added `TestSSHConfigBypass` (regression for Loop A). |
...
| `xmpd/history_backfill.py` | One-shot MPD log importer. Single public function `run_backfill(history_store, track_store, log_path, *, dry_run) -> dict[str,int]`. | Module-level constants: `LOG_LINE_RE`, `ISO_TIMESTAMP_RE`, `LEGACY_TIMESTAMP_RE`, `MPDCONF_LOG_FILE_RE`. Internal helpers: `_parse_played_at`, `_resolve_log_path`. Idempotent via in-memory dedup set from `get_plays()`. |
| `xmpd/daemon.py` | `XMPDaemon` constructor wires every subsystem; `run()` starts threads. IPC dispatcher routes socket commands to handler methods. | Construction sequence includes `HistoryStore`, `HistorySyncer`, `ThreadPoolExecutor(max_workers=1)` when `config['history']['enabled']` is True and `track_store is not None`. Passes all three into `HistoryReporter`. `run()` calls `startup_nudge()` after `_running=True`. `stop()` shuts executor before joining history thread. `ThreadPoolExecutor` and `as_completed` now at module level (was inline). `_cmd_history_json(args)` IPC handler (Phase 5) directly below `_cmd_search_json`. `_cmd_history_backfill(args)` IPC handler (Phase 6) below `_cmd_history_json`. `_autodetect_mpd_log_path()` helper walks `_MPDCONF_CANDIDATES` to find MPD log path. Dispatcher cases at lines ~694 (history-json) and ~697 (history-backfill). |
...
| `bin/xmpctl` | CLI client. Subcommand dispatch via top-level `if`/`elif` blocks; Unix socket IPC; `format_track_fzf` for ANSI rendering; `colorize` helpers. | `cmd_history_json(args)` handles `--mode`, `--since SPEC`, `--limit`, `--format`. `cmd_history_backfill(args)` handles `--log PATH`, `--dry-run` (Phase 6). Client-side SPEC translation (`30d`/`7d`/`1h`/`all` to ISO). `format_played_at(iso_str)` renders ISO as `May-12 19:39`. `format_duration_seconds(seconds)` renders as `m:ss`. Dispatch in `main()`. Help text includes `history-json` and `history-backfill`. |
...
| `bin/xmpd-doctor` | Multi-host healthcheck bash script. Read-only diagnostic; no daemon dependency. Reads local `~/.config/xmpd/history.db` via sqlite3 CLI; SSHes to WATCHTOWER for receiver version + doctor JSON. | Shebang `#!/usr/bin/env bash`, `set -uo pipefail` (no `-e`). Three sections: Local, Cluster (via WATCHTOWER), Per-host row state. Exit codes: 0 green, 2 yellow, 1 red (sticky). `WATCHTOWER_REACHABLE` flag set after timed SSH probe; guards section_cluster. jq fallback via `python3 -c` (detected by actual invocation, not `command -v`). Parses `tailscale_peers` (hostname/online) from receiver doctor JSON. All SSH invocations use `-F $HOME/.ssh/config` via `SSH_CONFIG` constant (Phase 8 fix: same OpenSSH 10.2 issue as history_syncer). |
| `bin/xmpd-history` | Bash + fzf wrapper for browsing local play history. Single-mode design (no Search/Browse split). | `start:reload(...)` invokes `xmpctl history-json --mode $(cat MODE_FILE) --since 30d --format fzf`. `ctrl-t` toggle flips temp file between `time`/`count` and reloads. `XMPD_HISTORY_MODE_FILE` env override is the test addressability seam. Bindings: enter=play, ctrl-q=queue, ctrl-r=radio, ctrl-l=like, tab=select, ctrl-a=queue-all, ctrl-p=clear+play-all. Uses `--tabstop=8` (no hyphen) for fzf 0.70.0 compatibility; does not redirect fzf stdin from `/dev/null` (Phase 8 fixes). |
| `tests/test_history_backfill.py` | 16 tests for `xmpd/history_backfill.py` and `cmd_history_backfill` in `bin/xmpctl`. | 5 classes: TestLogLineRegex (2), TestParsePlayedAt (4), TestRunBackfill (6), TestAutodetectLogPath (2), TestXmpctlCmdHistoryBackfill (2). Uses `history_store_temp` fixture. Asserts via raw `sqlite3.connect` + SELECT. |
| `tests/fixtures/sample_mpd_log` | 22-line MPD log fixture mixing ISO 8601 and legacy `MMM DD` formats. | 17 valid `player: played` matches, 6 orphan track IDs, 5 ignored lines (exception, decoder, random, non-played player, output). |
| `tests/test_xmpd_doctor.py` | Pytest harness for `bin/xmpd-doctor`. 11 tests (9 scenarios + 1 parametrized + 1 SSH config regression). | PATH-stub strategy: `_write_stub`, `_seed_db`, `_run_doctor` helpers. Stubs for tailscale, ssh, sqlite3, jq. Uses real sqlite3 subprocess to seed test DBs (never imports the bash script). Phase 8 added `test_ssh_commands_use_user_config` (regression for Loop E). |
| `tests/test_xmpctl_history_json.py` | 8 tests for `cmd_history_json` in `bin/xmpctl`. | Loads `bin/xmpctl` via `importlib.machinery.SourceFileLoader`. Tests: default args, since-all passthrough, SPEC translation, invalid-since exit, NDJSON output, fzf line shape, count mode play_count, daemon error exit. |
| `tests/test_xmpd_history.py` | 5 shell smoke tests for `bin/xmpd-history`. | fzf stubs extract and verify binding args. Tests: initial reload command, ctrl-t toggle via XMPD_HISTORY_MODE_FILE, clean exit on empty input. Phase 8 added `TestFzfStdinNotDevNull` (regression for Loop C /dev/null fix) and `TestFzfTabstopFlag` (regression for Loop C --tabstop fix). |
...
| `tests/test_daemon.py` | Daemon init, thread startup, socket commands, history wiring (7 tests in `TestHistoryWiring`), history-json IPC (5 tests in `TestCmdHistoryJson`). | `_config_with_history(tmp_path, enabled)` helper. TestCmdHistoryJson tests: disabled returns error, returns rows, invalid since, invalid mode, count mode. |
