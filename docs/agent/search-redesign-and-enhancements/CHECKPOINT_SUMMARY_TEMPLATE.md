...
> Filled in by `spark-deploy-verify` after code review passes. Skip this section if smoke
> harness is disabled for this feature. While review is pending or in progress, this section
> stays blank or marked `pending deploy-verify`.

- **Result**: PASS / FAIL / BROKEN / skipped (no surface-touching changes)
- **Diff paths matched against markers**: [list of changed paths that triggered the probe, or "n/a -- skipped"]
- **Command run**: [the smoke command]
- **Prerequisites started**: [list, or "none"]

### Failure Lines

> Skip if Result = PASS or skipped. Paste only the lines matching the configured failure format -- one per failing surface. No logs, no transcripts, no screenshots. Screenshots, if produced, are referenced by path only.

```
[Paste matched failure lines here]
```
...
[For each repaired helper, one or two sentences: what the root cause turned out to be, why the chosen repair (patch vs regeneration) was right, and whether the underlying problem (e.g. a CLI version bump) is likely to recur for other helpers.]

### Unlisted Helper Suggestions

> From phase summaries' "Helper Issues -> Unlisted helpers attempted" subsection. These are
> mechanical needs the planner did not allocate a helper for. Forward to the reviewer; do
> NOT author new helpers mid-feature without user approval.
...
[Observations, warnings, or context that the next batch's coding agents should know. This section is included in the shared prefix for the next batch's agent prompts.]
